    ...
    Pass {
        ...
        fragmentData vert (vertexData vd)
        {
            fragmentData fd;
			 ...
			 UNITY_SETUP_INSTANCE_ID(vd);                 // récupère l’index d’instance / d’œil
    UNITY_INITIALIZE_OUTPUT(fragmentData, fd);   // initialise fd à 0
    UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(fd);   // prépare les données stéréo
